...
from tensorflow_model_optimization.python.core.quantization.keras.experimental.ternarization import ternarization_quantize_scheme
...
class QuantizeNumericalTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def _get_sepconv1d_bn_relu_model(self):
    ...
    x = tf.keras.layers.SeparableConv1D(filters=5, kernel_size=3, strides=2)(i)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU()(x)
    return tf.keras.Model(i, x)
  ...
  def _get_sepconv1d_stacked_model(self):
    ...
    x = tf.keras.layers.SeparableConv1D(filters=5, kernel_size=3, strides=2)(i)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.SeparableConv1D(filters=5, kernel_size=3, strides=2)(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU()(x)
    return tf.keras.Model(i, x)
  ...
  @parameterized.parameters([
      _get_single_conv_model,
      _get_single_dense_model,
      _get_single_conv_relu_model,
      _get_stacked_convs_model,
      _get_conv_bn_relu_model,
      _get_depthconv_bn_relu_model,
      _get_separable_conv2d_model,
      _get_sepconv1d_bn_model,
      _get_sepconv1d_bn_relu_model,
  ...
  def testModelEndToEnd(self, model_fn):
    ...
    model = model_fn(self)
    model = quantize.quantize_apply(
        quantize.quantize_annotate_model(model),
        ternarization_quantize_scheme.TernarizationQuantizeScheme())
