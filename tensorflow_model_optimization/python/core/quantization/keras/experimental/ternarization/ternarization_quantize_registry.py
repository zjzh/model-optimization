...
class TernarizationQuantizeRegistry(quantize_registry.QuantizeRegistry,
  ...
  def __init__(self, disable_per_axis=False):
    ...
    # QuantizeConfig.
    self._layer_quantize_map[
        layers.Activation] = TernarizationActivationQuantizeConfig()

    self._layer_quantize_map[layers.Conv2DTranspose] = (
        TernarizationConvTransposeQuantizeConfig(['kernel'], ['activation'],
                                                 False))

    self._disable_per_axis = disable_per_axis
    if not self._disable_per_axis:
      self._layer_quantize_map[layers.Conv2D] = TernarizationConvQuantizeConfig(
          ['kernel'], ['activation'], False)
      self._layer_quantize_map[
          layers.DepthwiseConv2D] = TernarizationConvQuantizeConfig(
              ['depthwise_kernel'], ['activation'], False)
    else:
      self._layer_quantize_map[layers.Conv2D] = TernarizationQuantizeConfig(
          ['kernel'], ['activation'], False)
      self._layer_quantize_map[
          layers.DepthwiseConv2D] = TernarizationQuantizeConfig(
              ['depthwise_kernel'], ['activation'], False)
  ...
  def _get_quantize_config(self, layer_type):
    ...
    return TernarizationQuantizeConfig(quantize_info.weight_attrs,
  ...
  def get_quantize_config(self, layer):
    ...
    if self._is_rnn_layer(layer):
      ...
      return TernarizationQuantizeConfigRNN(weight_attrs, activation_attrs,
                                            False)
...
class TernarizationQuantizeConfig(QuantizeConfig):
  ...
  def set_quantize_activations(self, layer, quantize_activations):
    ...
    for activation_attr, activation in zip(self.activation_attrs,
...
class TernarizationQuantizeConfigRNN(TernarizationQuantizeConfig, _RNNHelper):
  ...
  def get_activations_and_quantizers(self, layer):
    ...
    for activation_attrs_cell, rnn_cell in zip(self.activation_attrs,
                                               self._get_rnn_cells(layer)):
...
class TernarizationActivationQuantizeConfig(QuantizeConfig):
  ...
  def _assert_activation_layer(self, layer):
    if not isinstance(layer, layers.Activation):
      raise RuntimeError(
          'TernarizationActivationQuantizeConfig can only be used with '
  ...
  def get_output_quantizers(self, layer):
    ...
    if not hasattr(layer.activation, '__name__'):
      raise ValueError('Activation {} not supported by '
                       'TernarizationActivationQuantizeConfig.'.format(
                           layer.activation))
...
class TernarizationConvQuantizeConfig(TernarizationQuantizeConfig):
  """QuantizeConfig for Conv2D/DepthwiseConv2D layers."""

  def __init__(self, weight_attrs, activation_attrs, quantize_output):
    super(TernarizationConvQuantizeConfig,
          self).__init__(weight_attrs, activation_attrs, quantize_output)

    self.weight_quantizer = ternarization_quantizers.TernarizationConvWeightsQuantizer(
    )
...
class TernarizationConvTransposeQuantizeConfig(TernarizationQuantizeConfig):
  ...
  def __init__(self, weight_attrs, activation_attrs, quantize_output):
    super(TernarizationConvTransposeQuantizeConfig,
          self).__init__(weight_attrs, activation_attrs, quantize_output)
...
def _types_dict():
  return {
      ...
      'TernarizationQuantizeConfigRNN':
          TernarizationQuantizeConfigRNN,
      'TernarizationActivationQuantizeConfig':
          TernarizationActivationQuantizeConfig,
