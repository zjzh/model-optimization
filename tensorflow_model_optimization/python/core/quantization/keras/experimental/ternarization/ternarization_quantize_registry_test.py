...
class QuantizeRegistryTest(tf.test.TestCase, parameterized.TestCase,
  ...
  def testGetsPerTensorIfPerAxisDisabled(self, disable_per_axis):
    ...
    if disable_per_axis:
      self.assertIsInstance(
          quantize_config,
          ternarization_quantize_registry.TernarizationQuantizeConfig)
    else:
      self.assertIsInstance(
          quantize_config,
          ternarization_quantize_registry.TernarizationConvQuantizeConfig)
  ...
  def testReturnsConfig_KerasRNNLayer(self):
    ...
    self._assert_activation_quantizers(activation_quantizers)
    self.assertEqual([layer.cell.activation, layer.cell.recurrent_activation],
                     activations)
  ...
  def testReturnsActivationConfig_Activation(self):
    ...
    self.assertIsInstance(
        quantize_config,
        ternarization_quantize_registry.TernarizationActivationQuantizeConfig)
...
class TernarizationQuantizeConfigTest(tf.test.TestCase, _TestHelper):
  ...
  def testGetsQuantizeWeightsAndQuantizers(self):
    ...
    quantize_config = ternarization_quantize_registry.TernarizationQuantizeConfig(
...
class TernarizationQuantizeConfigRNNTest(tf.test.TestCase, _TestHelper):
  ...
  def setUp(self):
    ...
    self.quantize_config = ternarization_quantize_registry.TernarizationQuantizeConfigRNN(
  ...
  def _expected_activations(self):
    return [
        self.cell1.activation, self.cell1.recurrent_activation,
...
class ActivationQuantizeConfigTest(tf.test.TestCase):
  ...
  def testRaisesErrorUnsupportedActivation(self):
    quantize_config = ternarization_quantize_registry.TernarizationActivationQuantizeConfig(
    )
