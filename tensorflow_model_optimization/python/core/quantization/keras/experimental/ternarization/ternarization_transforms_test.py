...
Conv2DBatchNormActivationQuantize = ternarization_transforms.Conv2DBatchNormActivationQuantize
Conv2DBatchNormReLUQuantize = ternarization_transforms.Conv2DBatchNormReLUQuantize
...
class DefaultTransformsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def _get_model(self, layer_type, squeeze_type, normalization_type,
                 activation_type):
    activation = None
  ...
  def _test_conv_squeeze_or_dense_bn_activation_transform(
      ...
      squeeze_type=None):
    model = self._get_model(layer_type, squeeze_type, normalization_type,
                            activation_type)
  ...
  def testConv2DBatchNormQuantize(self, layer_type, normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        ...
        transform_class=ternarization_transforms.Conv2DBatchNormQuantize,
        conv_or_dense_activation_class=quantize_aware_activation.NoOpActivation,
        normalization_quantize_config_class=ternarization_quantize_configs
  ...
  def testConv2DBatchNormReLUQuantize(self, layer_type, normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        ...
        transform_class=ternarization_transforms.Conv2DBatchNormReLUQuantize,
        conv_or_dense_activation_class=quantize_aware_activation.NoOpActivation,
        normalization_quantize_config_class=ternarization_quantize_configs
        .NoOpQuantizeConfig)
  ...
  def testConv2DBatchNormActivationQuantize(self, layer_type,
                                            normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        layer_type=layer_type,
        squeeze_type=None,
        ...
        transform_class=ternarization_transforms
  ...
  def testConv2DReshapeBatchNormQuantize(self, layer_type, normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        layer_type=layer_type,
        squeeze_type='sepconv1d_squeeze',
        normalization_type=normalization_type,
        activation_type=False,
        transform_class=ternarization_transforms.Conv2DReshapeBatchNormQuantize,
        conv_or_dense_activation_class=quantize_aware_activation.NoOpActivation,
        normalization_quantize_config_class=ternarization_quantize_configs
        .TernarizationOutputQuantizeConfig)
  ...
  def testConv2DReshapeBatchNormReLUQuantize(self, layer_type,
                                             normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        layer_type=layer_type,
        squeeze_type='sepconv1d_squeeze',
  ...
  def testConv2DReshapeBatchNormActivationQuantize(self, layer_type,
                                                   normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        layer_type=layer_type,
        squeeze_type='sepconv1d_squeeze',
  ...
  def testDenseBatchNormQuantize(self, layer_type, normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        ...
        normalization_type=normalization_type,
        activation_type=None,
        transform_class=ternarization_transforms.DenseBatchNormQuantize,
        conv_or_dense_activation_class=quantize_aware_activation.NoOpActivation,
        normalization_quantize_config_class=ternarization_quantize_configs
        .TernarizationOutputQuantizeConfig)
  ...
  def testDenseBatchNormReLUQuantize(self, layer_type, normalization_type):
    self._test_conv_squeeze_or_dense_bn_activation_transform(
        ...
        normalization_type=normalization_type,
        activation_type='relu',
        transform_class=ternarization_transforms.DenseBatchNormReLUQuantize,
        conv_or_dense_activation_class=quantize_aware_activation.NoOpActivation,
        normalization_quantize_config_class=ternarization_quantize_configs
        .NoOpQuantizeConfig)
  ...
  def testSeparableConv1DQuantize_(self, kwargs):
    ...
    sepconv_model = tf.keras.Sequential([
        tf.keras.Input(shape=(num_row, stack_size), batch_size=num_samples),
  ...
  @parameterized.named_parameters(('padding_valid', {
      'padding': 'valid'
  }), ('padding_same', {
      'padding': 'same'
  }), ('padding_same_dilation_2', {
  ...
  def testSeparableConvQuantize_(self, kwargs):
    ...
    for _ in range(epochs):
      ...
      self.assertAllClose(
          sepconv_model.predict(x), transformed_model.predict(x))

  @parameterized.parameters(
      ('relu', ternarization_transforms.LayerReLUQuantize),
      ('act_relu', ternarization_transforms.LayerReluActivationQuantize),
  )
  ...
  def testAddsQuantizeLayerAfterInputLayer(self):
    ...
    transformed_model, _ = ModelTransformer(
        model, [ternarization_transforms.InputLayerQuantize()]).transform()

    for input_layer in transformed_model._input_layers:
      layer_after_input = input_layer._outbound_nodes[0].outbound_layer
      self.assertIsInstance(layer_after_input, quantize_layer.QuantizeLayer)
  ...
  def testConcatTransform(self):
    ...
    _, updated_metadata = ModelTransformer(
        model, [ternarization_transforms.ConcatTransform()],
    ...
    self.assertEqual('TernarizationQuantizeConfig',
  ...
  def testConcatMultipleLevels(self):
    ...
    _, layer_metadata = ModelTransformer(
        model, [ternarization_transforms.ConcatTransform()]).transform()
